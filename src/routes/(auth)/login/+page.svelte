<script lang="ts">
	import { Button, Card } from '$lib/components/ui';
	import { page } from '$app/stores';

	const errorMessages: Record<string, string> = {
		not_authorized: 'Access denied. Please contact your administrator to get access.',
		invalid_state: 'Authentication failed. Please try again.',
		oauth_failed: 'Google sign-in failed. Please try again.'
	};

	const error = $derived($page.url.searchParams.get('error'));
	const errorMessage = $derived(error ? errorMessages[error] || 'An error occurred. Please try again.' : null);
</script>

<div class="min-h-screen bg-gray-100 flex items-center justify-center py-12 px-4">
	<Card class="w-full max-w-md">
		<div class="text-center mb-8">
			<div class="w-16 h-16 bg-indigo-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
				<svg class="w-10 h-10 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
				</svg>
			</div>
			<h1 class="text-2xl font-bold text-gray-900">Welcome back</h1>
			<p class="text-gray-500 mt-1">Sign in to your account</p>
		</div>

		{#if errorMessage}
			<div class="bg-red-50 text-red-700 px-4 py-3 rounded-lg mb-6 text-sm">
				{errorMessage}
			</div>
		{/if}

		<a href="/login/google" class="block">
			<Button type="button" variant="outline" class="w-full flex items-center justify-center gap-3">
				<svg class="w-5 h-5" viewBox="0 0 24 24">
					<path
						fill="#4285F4"
						d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
					/>
					<path
						fill="#34A853"
						d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
					/>
					<path
						fill="#FBBC05"
						d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
					/>
					<path
						fill="#EA4335"
						d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
					/>
				</svg>
				Sign in with Google
			</Button>
		</a>

		<p class="text-center text-sm text-gray-500 mt-6">
			Only pre-registered users can sign in.<br />
			Contact your administrator to get access.
		</p>
	</Card>
</div>
